...
message AType {
  float f = 1;
    ...
    def test_schema_registry_basic(self):
        ...
        and then will verify that schemas are replicated form the source to
        the shadow cluster
        """
        ...
        created_link.configurations.topic_metadata_sync_options.select_schema_registry_for_shadowing = True
        update_mask = google.protobuf.field_mask_pb2.FieldMask(
            paths=[
                "configurations.topic_metadata_sync_options.select_schema_registry_for_shadowing"
            ]
        )
        updated_link = self.update_link(created_link, update_mask)
        assert updated_link.configurations.topic_metadata_sync_options.select_schema_registry_for_shadowing, (
            "select_schema_registry_for_shadowing not set after update"
        )
