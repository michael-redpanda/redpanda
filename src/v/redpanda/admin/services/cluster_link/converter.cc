...
create_topic_metadata_mirroring_config(
  const topic_metadata_sync_options& options) {
    ...
    std::ranges::copy(
      options.get_mirrored_topic_properties(),
      std::inserter(
...
create_link_configuration(const create_cluster_link_request& req) {
    cluster_link::model::link_configuration config;
    config.topic_metadata_mirroring_cfg
      = create_topic_metadata_mirroring_config(
        req.get_cluster_link()
          .get_configuration()
...
create_authn_settings(const authentication_config& authn_config) {
    return authn_config.visit_authentication_oneof(
      ...
      [](std::monostate)
        -> cluster_link::model::connection_config::authn_variant {
          throw std::invalid_argument(
            "authentication_config is set but not provided");
      });
}
...
create_connection_config(const create_cluster_link_request& req) {
    cluster_link::model::connection_config config;
    const auto& client_options
      = req.get_cluster_link().get_configuration().get_client_options();
    ...
    if (client_options.has_authentication_config()) {
        config.authn_config = create_authn_settings(
          client_options.get_authentication_config());
    }
...
convert_create_to_metadata(create_cluster_link_request req) {
    ...
    try {
        auto& cluster_link = req.get_cluster_link();
