...
ss::future<> auto_topic_sensor::run_impl() {
    ...
    try {
        ...
        auto msg = ssx::sformat("Failed to udpate metadata: {}", e.what());
    ...
    try {
        ...
        vlog(logger().trace, "Describe topics repsonse: {}", response);
    } catch (const std::exception& e) {
    ...
    for (const auto& resp : response.data.results) {
        vlog(
          logger().trace,
        ...
        vlog(
          ...
          resp.resource_name,
          configs);
        auto it = candidate_topics.find(::model::topic{resp.resource_name});
        ...
        add_mirror_topic_cmds.emplace_back(model::add_mirror_topic_cmd{
          .topic = ::model::topic{resp.resource_name},
          .metadata = model::mirror_topic_metadata{
            .source_topic_name = ::model::topic{resp.resource_name},
    ...
    for (auto& c : add_mirror_topic_cmds) {
        vlog(logger().trace, "Adding mirror topic {}", c.topic);
        auto res = co_await get_link()->get_link_registry()->add_mirror_topic(
          get_link()->get_link_id(),
          std::move(c),
        ...
        vlog(logger().debug, "Successfully added mirror topic {}", c.topic);
    }
...
chunked_hash_map<::model::topic, auto_topic_sensor::topic_metadata>
auto_topic_sensor::find_candidate_topics() {
    auto& cluster = get_link()->get_cluster_connection();
    ...
    for (const auto& topic : topics) {
        ...
        auto partition_count = topic_cache.partition_count(topic);
        if (!partition_count.has_value() || *partition_count < 1) {
            vlog(logger().trace, "Skipping topic {} with no partitions", topic);
            continue;
        }

        auto rf = topic_cache.replication_factor(topic);
        if (!rf.has_value() || *rf < 1) {
            vlog(logger().trace, "Skipping topic {} with no RF", topic);
            continue;
        }
        ...
        vlog(
          ...
          topic,
          fmt::join(_config.filters.begin(), _config.filters.end(), ","));

        if (!::cluster_link::model::select_topic(topic, _config.filters)) {
            vlog(
              logger().trace,
        ...
        auto authorized_ops = topic_cache.authorized_operations_for_topic(
          topic);
        if (
          !authorized_ops.has_value()
          || *authorized_ops == kafka::topic_authorized_operations_not_set) {
            vlog(logger().trace, "Missing permissions for topic {}", topic);
            continue;
        }

        if ((*authorized_ops & required_permissions) != required_permissions) {
            vlog(
              logger().trace,
...
ss::future<kafka::describe_configs_response> auto_topic_sensor::describe_topics(
