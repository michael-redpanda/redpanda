...
ss::future<> topic_reconciler::do_reconcile_topic(model::id_t link_id) {
    ...
    for (const auto& [topic_name, mirror_topic_config] : mirror_topics) {
        ...
        if (local_topic_config.has_value()) {
            vlog(cllog.trace, "Topic {} exists, validating config", topic_name);
            auto err_msg = check_if_source_topic_is_invalid(
              local_topic_config.value(), mirror_topic_config);
            if (err_msg.has_value()) {
                vlog(
                  cllog.warn,
                  "Source topic {} is invalid: '{}'.  Marking mirror topic as "
                  "failed",
                  topic_name,
                  err_msg.value());
            ...
            auto update_cfg = maybe_create_update_mirror_topic(
              topic_name, local_topic_config.value(), mirror_topic_config);

            if (update_cfg.has_value()) {
                vlog(
                  cllog.debug,
                  "Detecting config change for topic {}.  Topic configs are {}",
                  topic_name,
                ...
                if (res != cluster::errc::success) {
                    ...
                    vlog(
                      cllog.debug,
                      "Successfully updated topic {} with new configs",
                      topic_name);
                }
            }
            ...
            if (
              ...
              > local_topic_config.value().partition_count) {
                vlog(
                  cllog.trace,
                  "Setting partition count for topic {} to {}",
                  topic_name,
                  mirror_topic_config.partition_count);
                ...
                if (res != cluster::errc::success) {
                    ...
                    vlog(
                      cllog.debug,
                      "Successfully created {} partitions for topic {}",
                      mirror_topic_config.partition_count,
                      topic_name);
                }
            }
            ...
            vlog(
              cllog.trace,
              "Topic {} does not exist, attempting creation",
              topic_name);
            ...
            if (res != cluster::errc::success) {
                ...
                vlog(
                  cllog.debug,
                  "Successfully created topic {} with {} partitions and "
                  "{} replication factor",
                  topic_name,
                  mirror_topic_config.partition_count,
                  mirror_topic_config.replication_factor);
