...
struct write_at_offset_stm_fixture
  : public raft::stm_raft_fixture<cluster_link::write_at_offset_stm> {
    std::tuple<ss::shared_ptr<cluster_link::write_at_offset_stm>> create_stms(
      state_machine_manager_builder& builder, raft_node_instance& node) {
        return builder.create_stm<cluster_link::write_at_offset_stm>(
          node.raft().get(),
          t_log,
          node.get_kvstore(),
    ...
    std::optional<ss::shared_ptr<cluster_link::write_at_offset_stm>>
    get_leader_stm() {
        ...
        return node(*leader_id)
          .raft()
          ->stm_manager()
          ->get<cluster_link::write_at_offset_stm>();
    }
};

TEST_F(write_at_offset_stm_fixture, test_write_at_offset_happy_path) {
...
TEST_F(write_at_offset_stm_fixture, test_write_at_offset_gaps) {
    enable_offset_translation();
    initialize_state_machines(3).get();
    wait_for_leader(10s).get();
    auto stm = get_leader_stm();
...
TEST_F(write_at_offset_stm_fixture, test_start_offset_advancement) {
    enable_offset_translation();
    initialize_state_machines(3).get();
    wait_for_leader(10s).get();
    auto stm = get_leader_stm();
...
TEST_F(write_at_offset_stm_fixture, test_concurrent_writes) {
    enable_offset_translation();
    initialize_state_machines(3).get();
    wait_for_leader(10s).get();
    auto stm = get_leader_stm();
...
TEST_F(write_at_offset_stm_fixture, test_writes_out_of_order) {
    enable_offset_translation();
    initialize_state_machines(3).get();
 ...
 */
TEST_F(write_at_offset_stm_fixture, test_concurrent_writers) {
    enable_offset_translation();
    initialize_state_machines(3).get();
    wait_for_leader(10s).get();
...
TEST_F(write_at_offset_stm_fixture, test_recovery_from_snapshot) {
    enable_offset_translation();
    initialize_state_machines(3).get();
    wait_for_leader(10s).get();
    auto stm = get_leader_stm();
...
TEST_F(write_at_offset_stm_fixture, test_failed_replication) {
    enable_offset_translation();
