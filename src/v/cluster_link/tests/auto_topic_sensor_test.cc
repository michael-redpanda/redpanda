...
TEST_F_CORO(auto_topic_sensor_test, select_all_filter) {
    co_await fixture()->upsert_link(get_default_metadata());
...
TEST_F_CORO(auto_topic_sensor_test, select_all_with_exclude) {
    ...
    md.state.topic_metadata_mirroring_cfg.filters.emplace_back(
      topic_filter_pattern{
        .pattern_type = filter_pattern_type::literal,
...
TEST_F_CORO(auto_topic_sensor_test, invalid_authorization) {
    fixture()->get_cluster_mock().add_topic(
      ::model::topic("test_topic"),
      3,
      3,
...
TEST_F_CORO(auto_topic_sensor_test, no_controller_present) {
...
TEST_F_CORO(auto_topic_sensor_test, topic_exists) {
...
TEST_F_CORO(auto_topic_sensor_test, topic_with_no_partitions) {
    fixture()->get_cluster_mock().add_topic(
      ::model::topic("test_topic"),
      0,
      3,
...
TEST_F_CORO(auto_topic_sensor_test, topic_with_no_replicas) {
    fixture()->get_cluster_mock().add_topic(
      ::model::topic("test_topic"),
      3,
      0,
...
class invalid_describe_configs_test : public auto_topic_sensor_test {
public:
    virtual ss::future<> SetUpAsync() override {
        co_await auto_topic_sensor_test::SetUpAsync();
...
class unsupported_describe_configs_test : public auto_topic_sensor_test {
public:
    virtual ss::future<> SetUpAsync() override {
        co_await auto_topic_sensor_test::SetUpAsync();
        fixture()->get_cluster_mock().register_handler(
