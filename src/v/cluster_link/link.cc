...
ss::future<cl_result<void>> link::register_task(task_factory* tf) {
    ...
    if (!t) {
        co_return err_info(
          ...
          ssx::sformat(
            "Failed to create task from factory {} for cluster "
            "link {}",
            tf->created_task_name(),
            _config.name));
    }
...
void link::update_config(
  model::metadata config, ::model::revision_id revision) {
    ...
    // if the link is not active or a specific topic in the link is
    // not active.
    if (!requires_active_replicators()) {
        ...
        for (const auto& [topic, _] : _config.state.mirror_topics) {
            ...
            vlog(
              cllog.debug,
              "Topic {} on link {} is not active, stopping its "
              "replicators",
              topic,
              _config.name);
...
ss::future<> link::handle_on_leadership_change(
  ...
  std::optional<::model::term_id> term) {
    ...
    if (mirror_topics.contains(ntp.tp.topic)) {
        vlog(
          cllog.debug,
          "[{}] Leadership change event for partition {}, "
          "is_leader: {}",
          _link_id,
          ntp,
          is_ntp_leader);
        ...
        if (!needs_replicators) {
            vlog(
              cllog.info,
              "[{}] Topic {} is not active, will stop any active "
              "replicators",
              _link_id,
              ntp.tp.topic);
        }
...
ss::future<cl_result<void>> link::do_register_task(std::unique_ptr<task> t) {
    ...
    cl_result<void> res = outcome::success();
    // Do not need to unregister the task callback as the task
    // lifetime is managed by the link
    ...
    // If we register a task after the link has started, then check
    // to see if it should start and do so
    if (!_as.abort_requested() && should_start_task(t.get())) {
    ...
    // Even if the task failed to start, still emplace it into the
    // list, the task reconcilier will re-attempt later
    auto name = t->name();
  ...
  chunked_vector<::model::topic> new_topics) {
    ...
    for (const auto& topic : new_topics) {
        ...
        if (!topic_cfg) {
            vlog(
              cllog.trace,
              "Topic {} does not exist yet.  Leadership changes "
              "will trigger "
              "replicators",
              topic);
            continue;
        ...
        for (auto p : std::views::iota(0, partition_count)) {
            ...
            if (!leader_node) {
                vlog(
                  cllog.trace,
                  "Topic {} partition {} does not have a leader "
                  "yet.  "
            ...
            if (!_manager->partition_manager().is_current_shard_leader(ntp)) {
                vlog(
                  cllog.trace,
                  "Topic {} partition {} is not on this shard.  "
                  "Skipping",
                  topic,
                  part_id);
                continue;
            ...
            if (!term.has_value()) {
                vlog(
                  cllog.trace,
                  "Topic {} partition {} does not have a term.  "
                  "Skipping",
                  topic,
                  part_id);
                continue;
