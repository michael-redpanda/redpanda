...
raft::replicate_stages partition::write_at_offset(
  ...
  model::timeout_clock::duration timeout) {
    if (!_write_at_offset_stm) {
        vlog(
          clusterlog.error,
          "Topic {} doesn't support write at offset",
          _raft->ntp());
        return raft::replicate_stages(
          raft::errc::write_at_offset_not_available);
    }
...
ss::future<> partition::start(
  ...
  const std::optional<xshard_transfer_state>& xst_state) {
    ...
    _write_at_offset_stm
      = _raft->stm_manager()->get<cluster_link::write_at_offset_stm>();
