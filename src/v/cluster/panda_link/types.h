...
struct metadata
  : serde::envelope<metadata, serde::version<0>, serde::compat_version<0>> {
    ...
    using panda_link_paused_t = ss::bool_class<struct panda_link_paused_tag>;
    /// Flag indicating if the panda link has been paused
    panda_link_paused_t paused{panda_link_paused_t::no};
...
template<>
struct fmt::formatter<panda_link::scram_credentials>
  : fmt::formatter<string_view> {
    auto format(const panda_link::scram_credentials& m, format_context& ctx)
      -> decltype(ctx.out());
};
...
template<>
struct fmt::formatter<panda_link::connection_config>
  : fmt::formatter<string_view> {
    auto format(const panda_link::connection_config& m, format_context& ctx)
      -> decltype(ctx.out());
};

template<>
struct fmt::formatter<panda_link::metadata> : fmt::formatter<string_view> {
    auto format(const panda_link::metadata& m, format_context& ctx)
      -> decltype(ctx.out());
};
