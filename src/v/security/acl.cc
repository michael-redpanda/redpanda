...
std::optional<acl_matches::acl_match> acl_matches::contains(
  acl_operation operation,
  const acl_principal& principal,
  const acl_host& host,
  acl_permission perm) const {
    for (const auto& entries : prefixes) {
        if (auto entry = entries.acl_entry_set.get().find(
            ...
            entry.has_value()) {
            return acl_matches::acl_match{entries.resource_pattern, *entry};
        }
    }
    ...
    if (wildcards) {
        if (auto entry = wildcards->acl_entry_set.get().find(
              operation, principal, host, perm);
            entry.has_value()) {
            return acl_matches::acl_match{wildcards->resource_pattern, *entry};
        }
    }
    ...
    if (literals) {
        if (auto entry = literals->acl_entry_set.get().find(
              operation, principal, host, perm);
            entry.has_value()) {
            return acl_matches::acl_match{literals->resource_pattern, *entry};
        }
    }
...
acl_store::find(resource_type resource, const ss::sstring& name) const {
    ...
    if (const auto it = _acls.find(wildcard_pattern); it != _acls.end()) {
        wildcards = opt_entry_set{{it->first, it->second}};
    }
    ...
    if (const auto it = _acls.find(literal_pattern); it != _acls.end()) {
        literals = opt_entry_set{{std::ref(it->first), std::ref(it->second)}};
    }
