...
public:
    ...
    bytes create_authn_message(
      const ss::sstring& username, const ss::sstring& password) {
        ...
        rv.push_back('\0');
        if (!username.empty()) {
            auto username_bytes = bytes::from_string(username);
            std::copy(
              std::make_move_iterator(username_bytes.begin()),
              std::make_move_iterator(username_bytes.end()),
              std::back_inserter(rv));
        }
        rv.push_back('\0');
        if (!password.empty()) {
            auto password_bytes = bytes::from_string(password);
            std::copy(
              std::make_move_iterator(password_bytes.begin()),
              std::make_move_iterator(password_bytes.end()),
              std::back_inserter(rv));
        }
        return rv;
    }
...
TEST_F_CORO(sasl_plain_test_fixture, test_authz_id) {
    ...
    std::copy(
      std::make_move_iterator(authn_msg_tmp.begin()),
      std::make_move_iterator(authn_msg_tmp.end()),
      std::back_inserter(authn_msg));
