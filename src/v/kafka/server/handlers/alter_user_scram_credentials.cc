...
std::pair<UpsertIter, DeleteIter> validate_range_no_duplicates(
  ...
  ErrIter out_it) {
    using upsert_type = typename UpsertIter::value_type;
    using delete_type = typename DeleteIter::value_type;
    ...
    auto valid_upsert_range_end = std::partition(
      upsert_begin, upsert_end, [&freq](const upsert_type& item) {
          return freq[item.name()] == 1;
      });

    auto valid_delete_range_end = std::partition(
      delete_begin, delete_end, [&freq](const delete_type& item) {
          return freq[item.name()] == 1;
      });
  ...
  request_context ctx, ss::smp_service_group) {
    ...
    for (const auto& u :
        ...
        if (user_exists) {
            ...
            vlog(klog.debug, "Created SCRAM credentials for user {}", u.name);
    ...
    const auto user_exists_and_same_scram_mech =
      [&ctx](const security::credential_user& u, scram_mechanism m) {
          auto mech = details::kafka_to_security_mechanism(m).value();
          try {
              auto maybe_creds
                = ctx.credentials().get<security::scram_credential>(u);
              ...
              switch (mech) {
              case security::scram_algorithm_t::sha256:
                  return creds.stored_key().size()
                         == security::scram_sha256::key_size;
              case security::scram_algorithm_t::sha512:
                  return creds.stored_key().size()
                         == security::scram_sha512::key_size;
              }
          } catch (...) {
              return false;
          }
