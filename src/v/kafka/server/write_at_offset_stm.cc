...
ss::future<write_at_offset_stm::errc> write_at_offset_stm::ensure_truncatable(
  ...
  std::optional<std::reference_wrapper<ss::abort_source>> as) {
    ...
    auto ghost_batches = model::make_compaction_placeholder_batches(
    ...
    chunked_vector<model::record_batch> to_replicate{
      std::from_range, std::move(ghost_batches) | std::views::as_rvalue};

    _inflight_last_offset = term_offset{
      .offset = effective_last_offset, .in_sync_term = _insync_term};
    auto stages = try_replicate_in_stages(std::move(to_replicate), as);
    auto failed = false;
    auto enqueue_f = co_await ss::coroutine::as_future(
      std::move(stages.request_enqueued));
    if (enqueue_f.failed()) {
        auto eptr = enqueue_f.get_exception();
        vlog(
          _log.warn,
          "Failed to enqueue replicate ghost batches for truncation up to "
          "offset {}: {}",
          new_start_offset,
          eptr);
        failed = true;
    }
    // ensure this is always awaited upon
    auto replicate_f = co_await ss::coroutine::as_future(
      std::move(stages.replicate_finished));
    if (replicate_f.failed()) {
        auto eptr = replicate_f.get_exception();
        vlog(
          _log.warn,
          "Failed to replicate ghost batches for truncation up to offset {}: "
          "{}",
          new_start_offset,
          eptr);
        failed = true;
...
auto fmt::formatter<kafka::write_at_offset_stm::errc>::format(
  ...
  -> decltype(ctx.out()) {
    ...
    return fmt::format_to(ctx.out(), "kafka::write_at_offset_stm::errc", msg);
}
